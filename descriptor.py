...
class ImageDescriptor:
    ...
    def __init__(self, color_space='HSV', bins_per_channel = 32):
    ...
    def compute_histogram(self, image):
        ...
        descriptor = np.concat(histograms)
        return descriptor
...
if __name__ == "__main__":
    ...
    descr = ImageDescriptor(color_space='RGB', bins_per_channel=64)
    img = cv2.imread("dataset/qsd1_w1/00003.jpg")
    hist = descr.compute_histogram(img)
    print(hist.shape)
    print(hist.max())
    print(hist.min())
    print(hist.mean())
